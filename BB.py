...
def homepage():
    ...
    def login():
        ...
        if username == "admin" and password == "123":
            ...
            login_message.place(x=65,y=420)
        else:
            login_message.config(text="Invalid username or password", fg="red")
            login_message.place(x=30,y=420)
    ...
    username_entry.pack()
    username_entry.place(x=30,y=250)
    ...
    password_entry.pack()
    password_entry.place(x=30,y=300)
    
    login_button = customtkinter.CTkButton(mainFrame, text="Sign In",hover_color="#eef2f3",text_color="#141414",fg_color="lightblue", command=login)
    login_button.pack()
    login_button.place(x=50,y=340)

    login_message = Label(mainFrame, text="")
    login_message.pack()

    def signup():
        webbrowser.open_new_tab("https://accounts.google.com/InteractiveLogin/identifier?continue=https%3A%2F%2Fmail.google.com%2Fmail%2Fu%2F0%2F&emr=1&followup=https%3A%2F%2Fmail.google.com%2Fmail%2Fu%2F0%2F&osid=1&passive=1209600&service=mail&ifkv=Af_xneHsGyA0zdo8B73Q-ejshj6Kn8Qu2GUQ6rITToA2rBmDJLwnJTEHO_VNvrymux5L-6BseIDX&flowName=GlifWebSignIn&flowEntry=ServiceLogin")
    signuptBtn = customtkinter.CTkButton(mainFrame,text="Sign Up",hover_color="#eef2f3",text_color="#141414",fg_color="lightblue",bg_color="#eef2f3",command=signup)
    signuptBtn.pack(padx=50,pady=50)
    signuptBtn.place(x=50,y=380)
...
def DailyPage():
    ...
    class ratings():
        Rate = tkinter.Label(mainFrame,background='#eef2f3', text="How are you feeling?")
        ...
        bb = tkinter.Label(mainFrame, fg='lightblue',text='ßß',background='#eef2f3',font=("Aerial regular",75))
        bb.place(x=70,y=260)
    ...
    class feedText():
        feedbackLabel = tkinter.Label(mainFrame,background='#eef2f3',text="Tell us why?")
...
def QuotePage(): 
    ...
    def colorz():
        color=rd.choice(["#fff"])
        return color
    display_quote=Label(mainFrame,text="Press the button below", height=2,padx=40,pady=250,wraplength=200,font=("Times",14,"bold"),bg="#fff")
    #display_quote.grid(row=0,column=0,stick="NW",padx=18,pady=15)
    display_quote.place(x=0,y=100)
    ...
    button=Button(mainFrame,text="Generate Quote",command=qgenerate,bg="#2718c9",font=("Aerial",14),fg="black",activebackground="#6b0e63",activeforeground="White")
...
def ScalePage():
    ...
    numScale = tkinter.Label(mainFrame,background="#eef2f3",foreground="#141414",text="0   1   2   3   4   5",font=('Arial bold',14))
    numScale.place(x=138, y=370)
